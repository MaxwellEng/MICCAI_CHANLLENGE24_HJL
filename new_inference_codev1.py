...
#from data_iter import PreprocessAdapterFromNpy, preprocessing_iterator_fromfiles, \
...
class nnUNetPredictor(object):
    ...
    def _internal_get_data_iterator_from_lists_of_filenames(self,
                                                            ...
                                                            num_processes: int, organ_size:str = 'big'):
        return preprocessing_iterator_fromfiles(input_list_of_lists, seg_from_prev_stage_files,
                                                ...
                                                self.verbose_preprocessing, organ_size)
    ...
    def predict_from_data_iterator_noexp(self,
                                   data_iterator,
                                   save_probabilities: bool = False,
                                   num_processes_segmentation_export: int = default_num_processes):
        """
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                #    r.append(export_prediction_from_logits(prediction, properties, self.configuration_manager, self.plans_manager, self.dataset_json, ofile, save_probabilities)
                    #    )
                    #)
                #else:
                    # convert_predicted_logits_to_segmentation_with_correct_shape(prediction, plans_manager,
                ...
                #        )
                #    )
                #r.append(convert_predicted_logits_to_segmentation_with_correct_shape(prediction, self.plans_manager, self.configuration_manager, self.label_manager, properties,   save_probabilities))
                #r.append( export_pool.starmap_async(
                #            convert_predicted_logits_to_segmentation_with_correct_shape, (
                #                (prediction, self.plans_manager,
                #                 self.configuration_manager, self.label_manager,
                #                 properties,
                ...
                print(f'done with {os.path.basename(ofile)}')
            #ret = [i.get()[0] for i in r]
            #adjust_ret = [change_one_hot_label(i,size) for i in ret ]
        if isinstance(data_iterator, MultiThreadedAugmenter):
            data_iterator._finish()
        ...
        # clear device cache
        empty_cache(self.device)
        #return adjust_ret, properties, ofile
        #return ret, properties, ofile
        return properties, ofile

    def predict_from_files_noexp(self,
                           list_of_lists_or_source_folder: Union[str, List[List[str]]],
                           output_folder_or_list_of_truncated_output_files: Union[str, None, List[str]],
                           save_probabilities: bool = False,
                           overwrite: bool = True,
                           ...
                           folder_with_segs_from_prev_stage: str = None,
                           num_parts: int = 1,
                           part_id: int = 0, organ_size: str = 'big'):
        """
        This is nnU-Net's default function for making predictions. It works best for batch predictions
        (predicting many images at once).
        """
        ...
        data_iterator = self._internal_get_data_iterator_from_lists_of_filenames(list_of_lists_or_source_folder,
                                                                                 seg_from_prev_stage_files,
                                                                                 output_filename_truncated,
                                                                                 num_processes_preprocessing, organ_size)

        return self.predict_from_data_iterator_noexp(data_iterator, save_probabilities, num_processes_segmentation_export)      
...
if __name__ == "__main__":
    ...
    input_path    = "./input/images/ct-angiography"
    result_folder = './output/images/aortic-branches'
    result1_path  = './result1/'
    result2_path  = './result2/'
