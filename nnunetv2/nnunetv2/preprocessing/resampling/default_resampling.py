...
#hjl        
def resample_data_or_seg_to_shape_faster(data: Union[torch.Tensor, np.ndarray],
                                  new_shape: Union[Tuple[int, ...], List[int], np.ndarray],
                                  current_spacing: Union[Tuple[float, ...], List[float], np.ndarray],
                                  new_spacing: Union[Tuple[float, ...], List[float], np.ndarray],
                                  is_seg: bool = False,
    ...
    data_reshaped = resample_data_or_seg_faster(data, new_shape, is_seg)
    return data_reshaped


def resample_data_or_seg_faster(data_torch: Union[torch.Tensor, np.ndarray], new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
                         is_seg: bool = False):
    """
    separate_z=True will resample with order 0 along z
    :param data:
    ...
    print("****The faster program is running!***")
    #dtype_data = data.dtype
    shape = np.array(data_torch[0].shape)
    new_shape = np.array(new_shape)
    if np.any(shape != new_shape):     
        #data = data.astype(float)
        ...
        reshaped_final_data = reshaped_final_data.to('cpu').numpy().astype(np.float16)
        ...
        # print("no resampling necessary")
        return data_torch.to('cpu').numpy().astype(np.float16)